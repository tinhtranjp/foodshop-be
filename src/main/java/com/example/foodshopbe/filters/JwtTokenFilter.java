package com.example.foodshopbe.filters;
...
public class JwtTokenFilter extends OncePerRequestFilter {
    ...
    try {
        ...
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "Umauthorized");
            return;
        }
        final String token = authHeader.substring(7);
        final String loginId = jwtTokenUtil.extractLoginId(token);
        if(loginId != null
                && SecurityContextHolder.getContext().getAuthentication() == null) {
            User userDetails = (User) userDetailsService.loadUserByUsername(loginId);
    ...
    private boolean isBypassToken(@NonNull HttpServletRequest request) {
        final List<Pair<String, String>> bypassTokens = Arrays.asList(
                ...
                Pair.of(String.format("%s/order1", apiPrefix),"GET"),
                Pair.of(String.format("%s/order1/**", apiPrefix),"GET"),
                ...
                Pair.of(String.format("%s/order1", apiPrefix),"POST"),
                Pair.of(String.format("%s/products/**", apiPrefix),"POST")
